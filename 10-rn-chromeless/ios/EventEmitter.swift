    ...
    switch eventName {
      ...
      case "timeupdate":
        listeners[eventName] = EventEmitter.player.addEventListener(type: PlayerEventTypes.TIME_UPDATE) { event in
          var currentTime: Double? = event.currentTime
          if currentTime != nil && currentTime!.isNaN {
            currentTime = nil
          }
          EventEmitter.sharedInstance.dispatch(name: event.type, body: ["currentTime": currentTime])
        }
