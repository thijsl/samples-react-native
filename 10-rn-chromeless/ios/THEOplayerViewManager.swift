...
@objc(THEOplayerViewManager) class THEOplayerViewManager: RCTViewManager {
  ...
  @objc
  func play() {
    playerView.theoplayer.play()
  }

  @objc
  func pause() {
    playerView.theoplayer.pause()
  }

  @objc
  func stop() {
    playerView.theoplayer.stop()
  }
  ...
  func fullscreenOff() {
    DispatchQueue.main.async { [weak self] in
      self?.playerView.theoplayer.presentationMode = .inline
    }
  }
  ...
  func getCurrentTime(_ resolve: @escaping RCTPromiseResolveBlock, _ reject: @escaping RCTPromiseRejectBlock) {
    return playerView.theoplayer.requestCurrentTime() { result, error in
      if error != nil {
  ...
  func setCurrentTime(_ newValue: NSNumber) {
    playerView.theoplayer.setCurrentTime(newValue.doubleValue)
  }
  ...
  func setSource(_ newValue: [String : Any]) {
    do {
      ...
      playerView.theoplayer.source = sourceDescription
    } catch {
      print(error)
    }
  }
